"""qos — marked voice / signaling / noise UDP streams.

This module owns the whole `qos` pipeline: its DSCP constants, its stream
worker, and its `qos` CLI command. qosgen.py just imports the command and
...
def qos_pipeline(dst, calls, signaling, signaling_dscp, noise, noise_multiplier, duration):
